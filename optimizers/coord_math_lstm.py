...
class CoordMathLSTM(nn.Module):
    ...
    def reset_state(self, optimizees: BaseOptimizee, step_size: float, **kwargs):
        ...
        self.init_vble_norm_smooth = self._get_init_vble_norm(optimizees)
    ...
    def _get_init_vble_norm(self, optimizees: BaseOptimizee):
        return torch.clamp(torch.linalg.norm(
            optimizees.X, ord=2, dim=(-2, -1), keepdim=True), min=1e-20)
    ...
    def forward(
        ...
        self.P, self.A = P * self.step_size, A
        return optimizees
