...
parser.add('--optimizee-type',
           choices=['QuadraticUnconstrained', 'LASSO', 'LogisticL1'],
...
if not opts.test:
    ...
    for i in range(num_test_batches):
        ...
        if opts.load_mat:
            optimizees.load_from_file(opts.optimizee_dir + '/' + str(i//slice) + '.mat', start_index, opts.test_batch_size)
            print("Loaded:", opts.optimizee_dir + '/' + str(i//slice) + '.mat', "Start: ", start_index, " Size: ", opts.test_batch_size)
            
        if opts.load_sol:
            optimizees.load_sol(opts.optimizee_dir + '/sol_' + str(i//slice) + '.mat', start_index, opts.test_batch_size)
            print("Sol Loaded.", i//slice, "Start: ", start_index, " Size: ", opts.test_batch_size)
